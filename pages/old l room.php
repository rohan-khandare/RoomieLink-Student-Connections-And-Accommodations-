<?php
   ...
   if ($_SERVER['REQUEST_METHOD'] == 'POST') {
      ...
      if (isset($_POST['security'])) {
          $facilities[] = 'SecuritySystem';
      }
      
      $facilitiesString = implode(',', $facilities);$_POST;

      // Insert into the `rooms` table
      // Adjust the prepared statement to match the number of placeholders
...
// Use bind_param with 11 "s" placeholders to match the 11 values you're inserting
      $stmt = $mysqli->prepare($sql);
      $stmt->bind_param('ssssssssss', $area, $rent, $capacity, $profession, $Contact, $Email, $gender, $facilitiesString, $nullVariable);


         //    $lastRoomID = $mysqli->insert_id; 
      
         //    $stmt->close();

         // Insert into the `roomfacilities` table
         // $sql = "INSERT INTO `roomfacilities` (`RoomID`, `Fridge`, `Geyser`, `AirConditioning`, `WashingMachine`, `Cupboard`, `Parking`, `SecuritySystem`) VALUES (NULL, ?, ?, ?, ?, ?, ?, ?, ?)";
         // $stmt = $mysqli->prepare($sql);

         // Bind parameters
         // $stmt->bind_param("isssssss", $lastRoomID, $Fridge, $Geyser, $AirConditioning, $WashingMachine, $Cupboard, $Parking, $SecuritySystem);

         if ($stmt->execute()) {
            echo '<script>alert("Your Room has listed successfully!");</script>';
         } else {
            echo '<p>Error inserting into roomfacilities: ' . $mysqli->error . '</p>';
         }
         ...
         <nav class="nav container">
            ...
            <div class="nav__menu" id="nav-menu">
               <ul class="nav__list">
                  ...
                  <li class="nav__item">
                     <a href="/WTCPNEW/pages/findroom.php" class="nav__link font-medium leading-6 text-black-600 transition duration-150 ease-out hover:text-gray-600">Find a Room</a>
                  </li>
                  <!-- <li class="nav__item">
